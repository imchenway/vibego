...
- “为什么 / 怎么做”按意图分流：概念为什么、安装升级、轻量用法说明默认简洁文本；行为/故障为什么默认生成 HTML 图。
- 具体功能、按钮、接口、配置、权限、构建、任务、数据或运行现场出现“为什么没反应 / 为什么失败 / 为什么没生效 / 为什么走错 / 为什么变慢 / 为什么不一致”，视为故障成因或逻辑链路理解，默认用 `vibe-diagram`。
- 解释具体对象、代码、文件更新、diff、模块、页面、接口、配置、数据、功能入口或运行结果时，默认用 `vibe-diagram` 生成单文件 HTML 图。
- 只在纯概念定义、翻译改写、一句话答案、简单命令或用户明确不要图时默认使用简洁文本；安装升级说明、轻量决策也可简洁文本，不再强制生成 HTML。
...
| `CODEX_GOALS_ENABLED` | 是否启动时追加 `-c features.goals=true`；默认 `1`，设为 `0/false/no/off` 可关闭实验性 `/goal` |
| `CODEX_SESSION_ROOT` | JSONL 根目录（默认 `~/.codex/sessions`）             |
| `CODEX_SESSION_GLOB` | JSONL 文件匹配（默认 `rollout-*.jsonl`）              |

Telegram worker 侧支持 Codex `/goal [objective|pause|resume|clear]`。裸 `/goal` 会打开轻量管理面板；
带参数的 `/goal <objective|pause|resume|clear>` 会直接透传给 Codex。Goal 的真实状态以 Codex active
thread 为准，vibego 不在本地数据库中复制状态。
...
Telegram 命令菜单可通过 `/plan_mode` 切换 Copilot 的 `interactive / plan / autopilot` 三态；排队发送默认使用 `Ctrl+Q`，立即发送仍使用 `Enter`。
...
| `COPILOT_QUEUE_SUBMIT_KEY` | 排队发送提交键，默认 `C-q`；若你的终端环境更适配 `Ctrl+Enter`，可改为 `C-Enter` |

## aiogram Worker 行为

- `/start`：返回 `chat_id`、`MODE`、`ACTIVE_MODEL`；日志打印 `chat_id` 与 `user_id`。
- `/session_live`：查看主会话与活动并行会话实况。
- `/plan_mode`：切换当前终端的 PLAN/MODE 状态。
- `/goal [objective|pause|resume|clear]`：仅 Codex worker 可用；裸 `/goal` 打开管理面板，带参数时用于查看、设置、暂停、恢复或清除当前
  Codex active thread 的目标。
- 文本消息：
