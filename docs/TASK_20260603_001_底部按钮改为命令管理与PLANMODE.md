...
本文件记录的是 `TASK_20260603_001` 当时“一行两列”的历史契约。后续 `TASK_20260603_002_Telegram普通消息立即排队模式切换.md` 已按用户确认把 Worker 底部键盘调整为一行三列：`📟 命令管理`、`🧭 PLAN/MODE`、`✉️ 立即/排队`。后续实现与排障以 `TASK_20260603_002` 和 `AGENTS.md` 最新 Facts Table 为准。

## 2026-06-03 后续变更提示（二）

后续 `TASK_20260603_005_普通消息默认排队并移除发送方式按钮.md` 又按用户确认移除了 `TASK_20260603_002` 引入的第三列 `✉️ 立即/排队`：Worker 底部主键盘当前最新契约为一行两列 `📟 命令管理` + `🧭 PLAN: ...` / `🧭 MODE: ...`，普通直聊 Codex/Copilot 默认 queued。后续实现与排障以 `TASK_20260603_005` 和 `AGENTS.md` 最新 Facts Table 为准。
