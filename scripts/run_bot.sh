...
SESSION_BINDER_TOKEN_FILE="${SESSION_BINDER_TOKEN_FILE:-$LOG_DIR/session_binder_token.txt}"
export SESSION_BINDER_TOKEN_FILE
...
# session_binder 默认常驻等待首个会话（0 表示一直等待），避免首次交互发生在启动较久后导致指针长期为空。
export SESSION_BINDER_TIMEOUT="${SESSION_BINDER_TIMEOUT:-0}"
