...
ENFORCED_AGENTS_NOTICE = (
    ...
    "先识别影响面，先跑受影响子项目 baseline（Verified 的 test / coverage / typecheck / build），"
    "baseline 不绿或 coverage 未达 Required Engineering Gate 就先修基线；新需求必须先补测试并确认失败，"
    "再写代码；写完后重新执行受影响测试、全量 test、coverage、typecheck/build，"
    "并连续两次通过且 coverage 恢复到目标值（默认 100%）；未达标可自动修复但最多 5 轮，超过即 fail-closed。"
    "测试资产属于长期验证资产，不得削弱、跳过或扩大 exclusions。"
    "最后输出影响项目、改动文件、测试清单、命令结果、coverage 结果、风险与回滚点。"
