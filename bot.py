...
TASK_BATCH_STATUS_START_CALLBACK = "task:batch_status:start"
TASK_BATCH_STATUS_TOGGLE_PREFIX = "task:batch_status:toggle:"
TASK_BATCH_STATUS_PAGE_PREFIX = "task:batch_status:page:"
TASK_BATCH_STATUS_LIMIT_PREFIX = "task:batch_status:limit:"
TASK_BATCH_STATUS_SELECT_PAGE_CALLBACK = "task:batch_status:select_page"
TASK_BATCH_STATUS_INVERT_PAGE_CALLBACK = "task:batch_status:invert_page"
TASK_BATCH_STATUS_CONFIRM_CALLBACK = "task:batch_status:confirm"
TASK_BATCH_STATUS_CANCEL_CALLBACK = "task:batch_status:cancel"
...
TASK_LIST_PAGE_SIZE_OPTIONS: tuple[int, ...] = (10, 20, 50)
...
DEFECT_PRECONDITION_LABEL = "前置条件"
DEFECT_REPRODUCTION_LABEL = "复现步骤"
DEFECT_CURRENT_STATE_LABEL = "现状"
DEFECT_EXPECTED_EFFECT_LABEL = "预期效果"
DEFECT_EXPECTED_RESULT_LABEL = "预期结果"
DEFECT_EXPECTED_RESULT_LEGACY_LABEL = "期望结果"
OPTIMIZE_CURRENT_EFFECT_LABEL = "当前效果"
OPTIMIZE_EXPECTED_EFFECT_LABEL = "期望效果"
DEFECT_DESCRIPTION_PATTERN = re.compile(
    rf"^{DEFECT_PRECONDITION_LABEL}：\n(?P<precondition>.*)\n\n{DEFECT_REPRODUCTION_LABEL}：\n(?P<reproduction>.*)\n\n{DEFECT_CURRENT_STATE_LABEL}：\n(?P<current_state>.*)\n\n{DEFECT_EXPECTED_EFFECT_LABEL}：\n(?P<expected_effect>.*)$",
    re.DOTALL,
)
DEFECT_DESCRIPTION_THREE_FIELD_PATTERN = re.compile(
    rf"^{DEFECT_PRECONDITION_LABEL}：\n(?P<precondition>.*)\n\n{DEFECT_REPRODUCTION_LABEL}：\n(?P<reproduction>.*)\n\n(?:{DEFECT_EXPECTED_RESULT_LABEL}|{DEFECT_EXPECTED_RESULT_LEGACY_LABEL})：\n(?P<expected_result>.*)$",
    re.DOTALL,
)
DEFECT_DESCRIPTION_LEGACY_PATTERN = re.compile(
...
def _build_structured_description(fields: Sequence[tuple[str, Optional[str]]]) -> str:
    """将多字段正文按统一格式组装为结构化文本。"""

    lines: list[str] = []
    for index, (label, value) in enumerate(fields):
        normalized_value = normalize_newlines(value or "").strip() or "-"
        if index > 0:
            lines.append("")
        lines.append(f"{label}：")
        lines.append(normalized_value)
    return "\n".join(lines)
...
def _build_defect_description(
    ...
    *,
    ...
    """构建缺陷任务正文；默认输出新四字段，必要时兼容旧三字段调用。"""

    if current_state is None and expected_effect is None and expected_result is None and reproduction is not None:
        # 兼容旧调用：_build_defect_description(reproduction, expected_result)
        expected_result = reproduction
        reproduction = precondition
        precondition = ""
    if current_state is None and expected_effect is None:
        return _build_structured_description(
            (
                (DEFECT_PRECONDITION_LABEL, precondition),
                (DEFECT_REPRODUCTION_LABEL, reproduction),
                (DEFECT_EXPECTED_RESULT_LABEL, expected_result),
            )
        )
    return _build_structured_description(
        (
            (DEFECT_PRECONDITION_LABEL, precondition),
            (DEFECT_REPRODUCTION_LABEL, reproduction),
            (DEFECT_CURRENT_STATE_LABEL, current_state),
            (DEFECT_EXPECTED_EFFECT_LABEL, expected_effect),
        )
    )


def _parse_defect_description_fields(description: Optional[str]) -> Optional[tuple[tuple[str, str], ...]]:
    """解析缺陷正文；新四字段走新口径，旧三字段与更老双字段保持兼容。"""
    ...
    if match:
        precondition = (match.group("precondition") or "").strip() or "-"
        reproduction = (match.group("reproduction") or "").strip() or "-"
        current_state = (match.group("current_state") or "").strip() or "-"
        expected_effect = (match.group("expected_effect") or "").strip() or "-"
        return (
            (DEFECT_PRECONDITION_LABEL, precondition),
            (DEFECT_REPRODUCTION_LABEL, reproduction),
            (DEFECT_CURRENT_STATE_LABEL, current_state),
    ...
    three_field_match = DEFECT_DESCRIPTION_THREE_FIELD_PATTERN.match(normalized)
    if three_field_match:
        precondition = (three_field_match.group("precondition") or "").strip() or "-"
        reproduction = (three_field_match.group("reproduction") or "").strip() or "-"
        expected_result = (three_field_match.group("expected_result") or "").strip() or "-"
        return (
            (DEFECT_PRECONDITION_LABEL, precondition),
            (DEFECT_REPRODUCTION_LABEL, reproduction),
            (DEFECT_EXPECTED_RESULT_LABEL, expected_result),
        )
    legacy_match = DEFECT_DESCRIPTION_LEGACY_PATTERN.match(normalized)
    if not legacy_match:
        return None
    reproduction = (legacy_match.group("reproduction") or "").strip() or "-"
    expected_result = (legacy_match.group("expected_result") or "").strip() or "-"
    return (
        (DEFECT_PRECONDITION_LABEL, "-"),
        (DEFECT_REPRODUCTION_LABEL, reproduction),
        (DEFECT_EXPECTED_RESULT_LABEL, expected_result),
    )


def _parse_defect_description(description: Optional[str]) -> Optional[tuple[str, ...]]:
    """解析缺陷结构化描述的值列表，供通用渲染逻辑复用。"""

    fields = _parse_defect_description_fields(description)
    if fields is None:
        return None
...
def _get_structured_task_labels(task_type: Optional[str]) -> Optional[tuple[str, ...]]:
    """返回结构化任务类型对应的字段标签。"""
...
def _get_structured_task_fields(
    task_type: Optional[str],
    description: Optional[str],
) -> Optional[tuple[tuple[str, str], ...]]:
    """返回结构化任务的“标签-值”列表，兼容新旧缺陷口径。"""

    normalized_task_type = _normalize_task_type(task_type)
    if normalized_task_type == "defect":
        return _parse_defect_description_fields(description)
    parsed_values = _parse_structured_task_description(normalized_task_type, description)
    labels = _get_structured_task_labels(normalized_task_type)
    if parsed_values is None or labels is None or len(parsed_values) != len(labels):
        return None
    return tuple(zip(labels, parsed_values))
    ...
    *values: Optional[str],
    ...
    if normalized_task_type == "defect":
        if not values:
            return None
        if len(values) == 2:
            return _build_defect_description(values[0], values[1])
        if len(values) >= 4:
            return _build_defect_description(
                values[0],
                values[1],
                current_state=values[2],
                expected_effect=values[3],
            )
        return _build_defect_description(values[0], values[1], values[2])
    if normalized_task_type == "task":
        if len(values) < 2:
            return None
        return _build_optimize_description(values[0], values[1])
    return None


def _parse_structured_task_description(task_type: Optional[str], description: Optional[str]) -> Optional[tuple[str, ...]]:
...
def _append_task_prompt_description_fields(
    ...
    *,
    ...
    """按任务类型输出推送提示词中的描述字段，结构化任务优先拆成多字段。"""

    normalized_task_type = _normalize_task_type(getattr(task, "task_type", None))
    structured_fields = _get_structured_task_fields(normalized_task_type, description)
    if structured_fields is not None:
        for label, value in structured_fields:
            _append_prompt_field_as_code_block(lines, label=label, value=value)
    else:
...
def _build_defect_confirm_summary_lines(
    *,
    ...
    _append_defect_summary_field(summary_lines, label=DEFECT_PRECONDITION_LABEL, value=precondition)
    _append_defect_summary_field(summary_lines, label=DEFECT_REPRODUCTION_LABEL, value=reproduction)
    _append_defect_summary_field(summary_lines, label=DEFECT_CURRENT_STATE_LABEL, value=current_state)
    _append_defect_summary_field(summary_lines, label=DEFECT_EXPECTED_EFFECT_LABEL, value=expected_effect)
    if isinstance(pending_attachments, Sequence):
        summary_lines.extend(
            _format_pending_attachments_for_create_summary(
                pending_attachments,
                reference_texts=(precondition, reproduction, current_state, expected_effect),
            )
        )
    else:
...
async def _build_task_create_confirm_summary_lines(
    *,
    ...
    if structured_fields is not None:
        for label, value in structured_fields:
            _append_defect_summary_field(summary_lines, label=label, value=value)
    elif description:
...
def _build_page_size_selector_row(
    current_limit: int,
    *,
    callback_data_builder: Callable[[int], str],
) -> list[list[InlineKeyboardButton]]:
    """构造每页条数切换按钮行。"""

    row: list[InlineKeyboardButton] = []
    for option in TASK_LIST_PAGE_SIZE_OPTIONS:
        label = f"{option}条/页"
        ...
        row.append(
            InlineKeyboardButton(
                text=label,
                callback_data=callback_data_builder(option),
            )
        )
    return [row]
...
def _format_task_detail(
    ...
    # 修复：描述字段智能清理预转义
    normalized_task_type = _normalize_task_type(getattr(task, "task_type", None))
    structured_fields = _get_structured_task_fields(normalized_task_type, task.description)
    if structured_fields is not None:
        if normalized_task_type == "defect" and len(structured_fields) == 4:
            icons = ("📌", "🧪", "🧭", "🎯")
        elif normalized_task_type == "defect":
            icons = ("📌", "🧪", "🎯")
        else:
            icons = ("🧭", "🎯")
        for icon, (label, value) in zip(icons, structured_fields):
            value_text = _format_task_detail_value(value)
            lines.append(f"{icon} {label}：{value_text}")
    else:
...
def _build_task_batch_status_keyboard() -> ReplyKeyboardMarkup:
    """批量修改状态：目标状态选择阶段菜单按钮。"""

    rows = [[KeyboardButton(text=_format_status(status)) for status in STATUS_DISPLAY_ORDER]]
    rows.append([KeyboardButton(text="取消")])
    _number_reply_buttons(rows)
    return ReplyKeyboardMarkup(keyboard=rows, resize_keyboard=True, one_time_keyboard=True)
...
def _build_task_batch_status_prompt() -> str:
    """批量改状态：构建目标状态选择提示文案。"""

    return "请选择本批任务要统一修改成的目标状态（发送“取消”可返回勾选列表）"


def _resolve_task_batch_status_choice(value: Optional[str]) -> Optional[str]:
    """解析批量改状态输入，兼容数字序号、状态标签与状态编码。"""

    display_to_status: dict[str, str] = {}
    for status in STATUS_DISPLAY_ORDER:
        label = _format_status(status)
        display_to_status[label] = status
        display_to_status[_strip_number_prefix(label)] = status
        if " " in label:
            display_to_status[label.split(" ", 1)[1].strip()] = status

    options = [_format_status(status) for status in STATUS_DISPLAY_ORDER]
    resolved = _resolve_reply_choice(value, options=[*options, "取消"])
    normalized = _normalize_choice_token(resolved or value)
    if normalized in display_to_status:
        return display_to_status[normalized]
...
async def _execute_task_push_from_state_data(
    *,
    task: TaskRecord,
    state: FSMContext,
    trigger_message: Message | CallbackQuery,
    data: Mapping[str, Any],
    supplement: Optional[str],
    push_mode: Optional[str],
    send_mode: Optional[str],
    pending_attachments: Optional[Sequence[Mapping[str, Any]]] = None,
) -> None:
    """根据 FSM 中已选参数执行最终推送。"""

    chat_id = data.get("chat_id") or (
        trigger_message.message.chat.id  # type: ignore[attr-defined]
        if isinstance(trigger_message, CallbackQuery) and trigger_message.message is not None
        else getattr(getattr(trigger_message, "chat", None), "id", None)
    )
    if not chat_id:
        await state.clear()
        target_message = trigger_message.message if isinstance(trigger_message, CallbackQuery) else trigger_message
        if target_message is not None:
            await target_message.answer("推送会话已失效，请重新点击按钮。", reply_markup=_build_worker_main_keyboard())
        return
    origin_message = data.get("origin_message")
    actor = data.get("actor") or (
        _actor_from_callback(trigger_message)
        if isinstance(trigger_message, CallbackQuery)
        else _actor_from_message(trigger_message)
    )
    dispatch_target = (data.get("dispatch_target") or "").strip()
    serialized_attachments = list(pending_attachments or [])
    if serialized_attachments:
        await _bind_serialized_attachments(task, serialized_attachments, actor=actor)

    await state.clear()
    if dispatch_target == PUSH_TARGET_PARALLEL:
        if isinstance(trigger_message, CallbackQuery):
            await trigger_message.answer("已进入并行分支选择")
    ...
    try:
        ...
        target_message = trigger_message.message if isinstance(trigger_message, CallbackQuery) else trigger_message
        if target_message is not None:
            await target_message.answer(str(exc), reply_markup=_build_worker_main_keyboard())
        return
    ...
    try:
        ...
        target_message = trigger_message.message if isinstance(trigger_message, CallbackQuery) else trigger_message
        if isinstance(trigger_message, CallbackQuery):
            await trigger_message.answer("推送失败：缺少模板配置", show_alert=True)
        elif target_message is not None:
            await target_message.answer("推送失败：缺少模板配置", reply_markup=_build_worker_main_keyboard())
        return
    if not success:
        target_message = trigger_message.message if isinstance(trigger_message, CallbackQuery) else trigger_message
        if isinstance(trigger_message, CallbackQuery):
            await trigger_message.answer("推送失败：模型未就绪", show_alert=True)
        elif target_message is not None:
            await target_message.answer("推送失败：模型未就绪", reply_markup=_build_worker_main_keyboard())
        return

    preview_block, preview_parse_mode = _wrap_text_in_code_block(prompt)
    if isinstance(trigger_message, CallbackQuery):
        await trigger_message.answer("已推送到模型")
    target_message = trigger_message.message if isinstance(trigger_message, CallbackQuery) else trigger_message
    await _send_model_push_preview(
        chat_id,
        preview_block,
...
async def _restore_task_batch_status_view(
    *,
    target_message: Optional[Message],
    fallback_message: Message,
    status: Optional[str],
    ...
    text, markup = await _build_task_batch_status_view(
        status=status,
        page=page,
        limit=limit,
        selected_task_ids=selected_task_ids,
    ...
    view_state = _make_batch_status_view_state(
        status=status,
        page=page,
        limit=limit,
        selected_task_ids=selected_task_ids,
...
def _format_task_batch_status_summary(
    *,
    task_ids: Sequence[str],
    target_status: str,
    success_items: Sequence[str],
    ...
    lines = [
        "*批量修改任务状态结果*",
        f"目标状态：{_format_status(target_status)}",
        (
            f"总览：{len(task_ids)} 个任务｜成功 {len(success_items)}"
            f"｜失败 {len(failed_items)}｜跳过 {len(skipped_items)}"
        ),
    ...
    if success_items:
        lines.append("")
        lines.append(f"✅ 已更新（{len(success_items)}）")
        lines.extend(f"- /{task_id}" for task_id in success_items)
    if skipped_items:
        lines.append("")
        lines.append(f"⏭️ 已跳过（{len(skipped_items)}）")
...
async def _execute_task_batch_status_update(
    *,
    trigger_message: Message,
    state: FSMContext,
    target_status: str,
    ...
    data = await state.get_data()
    task_ids = [str(item).strip() for item in (data.get("batch_status_task_ids") or []) if str(item).strip()]
    origin_message = data.get("batch_status_origin_message")
    actor = data.get("actor") or _actor_from_message(trigger_message)
    status = data.get("batch_status_status")
    page = int(data.get("batch_status_page", 1) or 1)
    limit = int(data.get("batch_status_limit", DEFAULT_PAGE_SIZE) or DEFAULT_PAGE_SIZE)
    ...
    for task_id in task_ids:
        ...
        if task.status == target_status:
            skipped_items.append((task_id, "已是目标状态"))
            continue
        try:
            ...
            failed_items.append((task_id, str(exc)))
            continue
        success_items.append(updated.id)
    ...
    summary = _format_task_batch_status_summary(
        task_ids=task_ids,
...
async def _render_task_view_from_state(state: TaskViewState) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if state.kind == "batch_status":
        status = state.data.get("status")
        page = int(state.data.get("page", 1) or 1)
        limit = int(state.data.get("limit", DEFAULT_PAGE_SIZE) or DEFAULT_PAGE_SIZE)
        selected_task_ids = [str(item).strip() for item in (state.data.get("selected_task_ids") or []) if str(item).strip()]
        selected_order = [str(item).strip() for item in (state.data.get("selected_task_order") or []) if str(item).strip()]
        return await _build_task_batch_status_view(
            status=status,
            page=page,
            limit=limit,
            selected_task_ids=selected_task_ids,
...
def _make_batch_status_view_state(
    *,
    status: Optional[str],
    page: int,
    limit: int,
...
async def on_tasks_help(message: Message) -> None:
    text = (
        ...
        "- /task_new 标题 | type=缺陷 | precondition=前置条件 | reproduction=复现步骤 | current_state=现状 | expected_effect=预期效果 — 创建结构化缺陷\n"
        "- /task_new 标题 | type=缺陷 | ... | expected_result=预期结果 — 兼容旧参数，仍按新四字段创建\n"
    ...
    *,
    ...
    rows.append(
        [
            ...
            InlineKeyboardButton(
                text="🔁 批量修改状态",
                callback_data=TASK_BATCH_STATUS_START_CALLBACK,
            ),
        ]
    )
    rows.append(
...
async def _build_task_batch_status_view(
    *,
    status: Optional[str],
    page: int,
    limit: int,
    ...
    lines = [
        ...
        f"筛选状态：{status_text} · 页码 {current_page_display}/{display_pages} · 每页 {limit} 条 · 总数 {total}",
        f"已选任务：{len(selection_order)} 个",
        "点击任务可切换勾选；确认后统一选择目标状态，并按勾选顺序逐个执行。",
    ]
    if not tasks:
        lines.append("当前页没有可选择的任务。")
    text = "\n".join(lines)
    ...
    for task in tasks:
        ...
        rows.append(
            [
                InlineKeyboardButton(
                    text=label[:64],
                    callback_data=f"{TASK_BATCH_STATUS_TOGGLE_PREFIX}{normalized_task_id}",
                )
            ]
        )
    ...
    if page > 1:
        nav_row.append(
            InlineKeyboardButton(
                text="⬅️ 上一页",
                callback_data=f"{TASK_BATCH_STATUS_PAGE_PREFIX}{page-1}",
            )
        )
    if total_pages and page < total_pages:
        nav_row.append(
    ...
    rows.extend(
        _build_page_size_selector_row(
            limit,
            callback_data_builder=lambda option: f"{TASK_BATCH_STATUS_LIMIT_PREFIX}{option}",
        )
    )
    ...
    if tasks:
        rows.append(
            [
                InlineKeyboardButton(
                    text="✅ 全选当前页",
                    callback_data=TASK_BATCH_STATUS_SELECT_PAGE_CALLBACK,
                ),
                InlineKeyboardButton(
                    text="🔁 反选当前页",
                    callback_data=TASK_BATCH_STATUS_INVERT_PAGE_CALLBACK,
                ),
            ]
        )
    ...
    rows.append(
        [
            InlineKeyboardButton(
                text=f"🚀 确认批量修改状态（{len(selection_order)}）",
                callback_data=TASK_BATCH_STATUS_CONFIRM_CALLBACK,
            )
        ]
    )
    rows.append(
        [
            InlineKeyboardButton(
                text="❌ 取消批量修改状态",
                callback_data=TASK_BATCH_STATUS_CANCEL_CALLBACK,
            )
        ]
    )
    return text, InlineKeyboardMarkup(inline_keyboard=rows)
...
async def _parse_defect_excel_import_file(path: Path) -> tuple[list[dict[str, Any]], list[str], int]:
    ...
    for row_index, row in enumerate(worksheet.iter_rows(min_row=2, values_only=True), start=2):
        if use_legacy_headers:
            title, precondition, reproduction, expected_effect, related_task_code, priority = (
        ...
        valid_rows.append(
            {
                ...
                "precondition": (str(precondition).strip() if precondition is not None else ""),
                "reproduction": (str(reproduction).strip() if reproduction is not None else ""),
                "current_state": (str(current_state).strip() if current_state is not None else ""),
...
def _extract_task_batch_status_view_state(message: Optional[Message]) -> Optional[TaskViewState]:
    """读取当前消息绑定的批量状态修改视图状态。"""
    ...
    if chat is None:
        return None
    state = _peek_task_view(chat.id, message.message_id)
    if state is None or state.kind != "batch_status":
        return None
    return state
...
@router.callback_query(F.data == TASK_BATCH_STATUS_START_CALLBACK)
async def on_task_batch_status_start(callback: CallbackQuery) -> None:
    """从任务列表进入批量状态修改勾选模式。"""
    ...
    next_state = _make_batch_status_view_state(
        status=status,
        page=page,
        limit=limit,
        selected_task_ids=[],
    ...
    await callback.answer("已进入批量状态修改模式")


@router.callback_query(F.data.startswith(TASK_BATCH_STATUS_TOGGLE_PREFIX))
async def on_task_batch_status_toggle(callback: CallbackQuery) -> None:
    """在批量状态修改视图中切换任务选择状态。"""

    message = callback.message
    view_state = _extract_task_batch_status_view_state(message)
    if view_state is None or message is None:
        await callback.answer("批量状态修改视图已失效，请重新进入。", show_alert=True)
        return
    task_id = _normalize_task_id((callback.data or "")[len(TASK_BATCH_STATUS_TOGGLE_PREFIX) :])
    if not task_id:
    ...
    page = int(view_state.data.get("page", 1) or 1)
    limit = int(view_state.data.get("limit", DEFAULT_PAGE_SIZE) or DEFAULT_PAGE_SIZE)
    await _restore_task_batch_status_view(
        target_message=message,
        fallback_message=message,
        status=status,
        page=page,
...
@router.callback_query(F.data.startswith(TASK_BATCH_STATUS_PAGE_PREFIX))
async def on_task_batch_status_page(callback: CallbackQuery) -> None:
    """批量状态修改勾选视图分页。"""
    ...
    if view_state is None or message is None:
        await callback.answer("批量状态修改视图已失效，请重新进入。", show_alert=True)
        return
    page_text = (callback.data or "")[len(TASK_BATCH_STATUS_PAGE_PREFIX) :].strip()
    try:
...
@router.callback_query(F.data.startswith(TASK_BATCH_STATUS_LIMIT_PREFIX))
async def on_task_batch_status_limit(callback: CallbackQuery) -> None:
    """批量状态修改视图切换每页条数。"""
    ...
    if view_state is None or message is None:
        await callback.answer("批量状态修改视图已失效，请重新进入。", show_alert=True)
        return
    limit_text = (callback.data or "")[len(TASK_BATCH_STATUS_LIMIT_PREFIX) :].strip()
    try:
        target_limit = int(limit_text)
    except ValueError:
        await callback.answer("分页数量参数错误", show_alert=True)
        return
    if target_limit not in TASK_LIST_PAGE_SIZE_OPTIONS:
        await callback.answer("不支持的分页数量", show_alert=True)
        return
    status = view_state.data.get("status")
    _selected_set, selected_order = _extract_task_batch_push_selection(view_state)
    await _restore_task_batch_status_view(
    ...
    await callback.answer(f"已切换为每页 {target_limit} 条")


async def _apply_task_batch_status_page_shortcut(
    callback: CallbackQuery,
    *,
    action: Literal["select", "invert"],
    success_notice: str,
) -> None:
    """对当前页可见任务应用批量状态修改快捷操作。"""
...
@router.callback_query(F.data == TASK_BATCH_STATUS_SELECT_PAGE_CALLBACK)
async def on_task_batch_status_select_page(callback: CallbackQuery) -> None:
    """批量状态修改勾选视图：全选当前页。"""

    await _apply_task_batch_status_page_shortcut(
        callback,
        action="select",
...
@router.callback_query(F.data == TASK_BATCH_STATUS_INVERT_PAGE_CALLBACK)
async def on_task_batch_status_invert_page(callback: CallbackQuery) -> None:
    """批量状态修改勾选视图：反选当前页。"""
...
@router.callback_query(F.data == TASK_BATCH_STATUS_CANCEL_CALLBACK)
async def on_task_batch_status_cancel(callback: CallbackQuery) -> None:
    """退出批量状态修改勾选模式并恢复任务列表。"""

    message = callback.message
    view_state = _extract_task_batch_status_view_state(message)
    if view_state is None or message is None:
        await callback.answer("批量状态修改视图已失效，请重新打开任务列表。", show_alert=True)
        return
    status = view_state.data.get("status")
    page = int(view_state.data.get("page", 1) or 1)
    limit = int(view_state.data.get("limit", DEFAULT_PAGE_SIZE) or DEFAULT_PAGE_SIZE)
    ...
    await callback.answer("已取消批量状态修改")


@router.callback_query(F.data == TASK_BATCH_STATUS_CONFIRM_CALLBACK)
async def on_task_batch_status_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    """确认勾选结果，进入统一状态选择流程。"""
    ...
    await state.clear()
    await state.update_data(
        batch_status_task_ids=selected_task_ids,
        batch_status_origin_message=message,
        batch_status_status=view_state.data.get("status"),
        batch_status_page=int(view_state.data.get("page", 1) or 1),
        batch_status_limit=int(view_state.data.get("limit", DEFAULT_PAGE_SIZE) or DEFAULT_PAGE_SIZE),
        actor=_actor_from_callback(callback),
    )
    await state.set_state(TaskBatchStatusStates.waiting_status)
    await callback.answer("请选择目标状态")
    await message.answer(
        _build_task_batch_status_prompt(),
        reply_markup=_build_task_batch_status_keyboard(),
    )
...
def _prepare_task_new_command_description(
    ...
    *,
    ...
    normalized_task_type = _normalize_task_type(task_type)
    if normalized_task_type == "defect":
        precondition = extra.get("precondition")
        reproduction = extra.get("reproduction")
        current_state = extra.get("current_state")
        expected_effect = extra.get("expected_effect")
        legacy_expected_result = extra.get("expected_result")
        if (
            precondition is not None
            or reproduction is not None
            or current_state is not None
            or expected_effect is not None
            or legacy_expected_result is not None
        ):
            return (
                _build_defect_description(
                    _normalize_command_field(precondition, DEFECT_PRECONDITION_LABEL),
                    _normalize_command_field(reproduction, DEFECT_REPRODUCTION_LABEL),
                    current_state=_normalize_command_field(current_state, DEFECT_CURRENT_STATE_LABEL),
                    expected_effect=_normalize_command_field(
                        expected_effect if expected_effect is not None else legacy_expected_result,
                        DEFECT_EXPECTED_EFFECT_LABEL,
                    ),
                ),
                pending_attachments,
...
async def _advance_task_create_to_description(message: Message, state: FSMContext) -> None:
    ...
    await state.update_data(processed_media_groups=[])
    if task_type == "defect":
        await state.set_state(TaskCreateStates.waiting_precondition)
        await message.answer(
            "请输入前置条件（可选），可直接发送图片/文件作为附件；若暂无可发送“跳过”继续（仅发送附件也会进入下一步）：",
            reply_markup=_build_description_keyboard(),
        )
        return
    await state.set_state(TaskCreateStates.waiting_description)
...
async def _handle_task_create_final_structured_field(
    message: Message,
    state: FSMContext,
    *,
    task_type: str,
    ...
    """处理 `/task_new` 结构化任务的最后一个字段录入，并生成确认摘要。"""
...
@router.message(TaskCreateStates.waiting_precondition)
async def on_task_create_precondition(message: Message, state: FSMContext) -> None:
    """缺陷创建：录入前置条件。"""
    ...
    await _handle_task_create_first_structured_field(
        ...
        next_state=TaskCreateStates.waiting_reproduction,
        next_prompt="请输入复现步骤（可选），可直接发送图片/文件作为附件；若暂无可发送“跳过”继续（仅发送附件也会进入下一步）：",
        reprompt_text="前置条件可选：可继续输入条件（可同时发送附件），或发送“跳过”继续录入复现步骤：",
    )
...
async def on_task_create_reproduction(message: Message, state: FSMContext) -> None:
    ...
    await _handle_task_create_first_structured_field(
        ...
        next_state=TaskCreateStates.waiting_current_state,
        next_prompt="请输入现状（可选），可直接发送图片/文件作为附件；若暂无可发送“跳过”继续（仅发送附件也会进入下一步）：",
        reprompt_text="复现步骤可选：可继续输入步骤（可同时发送附件），或发送“跳过”继续录入现状：",
    )


@router.message(TaskCreateStates.waiting_current_state)
async def on_task_create_current_state(message: Message, state: FSMContext) -> None:
    """缺陷创建：录入现状。"""
    ...
    await _handle_task_create_first_structured_field(
        ...
        field_label=DEFECT_CURRENT_STATE_LABEL,
        next_state=TaskCreateStates.waiting_defect_expected_effect,
        next_prompt="请输入预期效果（可选），可直接发送图片/文件作为附件；若暂无可发送“跳过”继续（仅发送附件也会进入下一步）：",
        reprompt_text="现状可选：可继续输入现状（可同时发送附件），或发送“跳过”继续录入预期效果：",
    )


@router.message(TaskCreateStates.waiting_defect_expected_effect)
async def on_task_create_defect_expected_effect(message: Message, state: FSMContext) -> None:
    """缺陷创建：录入预期效果并进入确认。"""

    await _handle_task_create_final_structured_field(
        message,
        state,
        task_type="defect",
        previous_keys=("precondition", "reproduction", "current_state"),
        current_key="expected_effect",
        current_label=DEFECT_EXPECTED_EFFECT_LABEL,
        reprompt_text="预期效果可选：可继续输入目标效果（可同时发送附件），或发送“跳过”进入确认创建：",
    )
...
async def on_task_create_expected_effect(message: Message, state: FSMContext) -> None:
    ...
    await _handle_task_create_final_structured_field(
        ...
        state,
        task_type="task",
        previous_keys=("current_effect",),
        current_key="expected_effect",
        current_label=OPTIMIZE_EXPECTED_EFFECT_LABEL,
...
async def on_task_create_confirm(message: Message, state: FSMContext) -> None:
    ...
    if extra_attachments or (extra_text and not is_cancel and not is_confirm):
        ...
        structured_fields = _get_structured_task_fields(task_type_code, description)
        update_label = structured_fields[-1][0] if structured_fields else "描述"
        ...
        if attachments_for_value:
            pending.extend(_serialize_saved_attachment(item) for item in attachments_for_value)
        if structured_fields is not None:
            updated_values = [value for _, value in structured_fields]
...
async def on_task_push_model_dispatch_target(message: Message, state: FSMContext) -> None:
    ...
    # 切入并行分支流后，后续文本将用于分支选择/前缀输入，不能再被“处理方式”FSM 拦截。
    await state.clear()
    await _begin_parallel_launch(
        task=task,
        chat_id=data.get("chat_id") or message.chat.id,
...
@router.message(TaskBatchStatusStates.waiting_status)
async def on_task_batch_status_choice(message: Message, state: FSMContext) -> None:
    """批量修改状态：统一选择目标状态后执行。"""

    data = await state.get_data()
    task_ids = data.get("batch_status_task_ids") or []
    if not task_ids:
        await state.clear()
        await message.answer("批量状态修改会话已失效，请重新勾选任务。", reply_markup=_build_worker_main_keyboard())
        return
    raw_text = message.text or ""
    if _is_cancel_message(raw_text) or _strip_number_prefix(raw_text) == "取消":
        await state.clear()
        await _restore_task_batch_status_view(
            target_message=data.get("batch_status_origin_message"),
            fallback_message=message,
            status=data.get("batch_status_status"),
            page=int(data.get("batch_status_page", 1) or 1),
            limit=int(data.get("batch_status_limit", DEFAULT_PAGE_SIZE) or DEFAULT_PAGE_SIZE),
        ...
        await message.answer("已取消批量状态修改。", reply_markup=_build_worker_main_keyboard())
        return
    target_status = _resolve_task_batch_status_choice(raw_text)
    if target_status is None:
        await message.answer(
            "请选择一个有效的目标状态，发送“取消”可返回勾选列表：",
            reply_markup=_build_task_batch_status_keyboard(),
        )
        return
    ...
    await message.answer(
        (
            f"开始批量修改 {len(task_ids)} 个任务的状态。\n"
            f"目标状态：{_format_status(target_status)}"
        ),
        reply_markup=_build_worker_main_keyboard(),
    )
    await _execute_task_batch_status_update(
        trigger_message=message,
        state=state,
        target_status=target_status,
    )
        ...
        try:
            defect_task = await TASK_SERVICE.create_root_task(
                ...
                description=_build_defect_description(
                    str(row.get("precondition") or "").strip(),
...
async def on_task_push_model_send_mode(message: Message, state: FSMContext) -> None:
    ...
    await _execute_task_push_from_state_data(
        ...
        trigger_message=message,
        data=data,
        supplement=data.get("pending_push_supplement"),
        push_mode=push_mode or None,
        send_mode=send_mode,
        pending_attachments=data.get("pending_push_attachments") or [],
    )
...
async def on_task_push_model_supplement(message: Message, state: FSMContext) -> None:
    ...
    await _execute_task_push_from_state_data(
        ...
        push_mode=push_mode or None,
        send_mode=send_mode,
        pending_attachments=serialized,
    )
...
async def on_task_defect_report_title(message: Message, state: FSMContext) -> None:
    ...
    await state.set_state(TaskDefectReportStates.waiting_precondition)
    await message.answer(
        "请输入前置条件（可选），可直接发送图片/文件作为附件；若暂无可发送“跳过”继续（仅发送附件也会进入下一步）：",
        reply_markup=_build_description_keyboard(),
    )
...
@router.message(TaskDefectReportStates.waiting_precondition)
async def on_task_defect_report_precondition(message: Message, state: FSMContext) -> None:
    """报告缺陷：处理前置条件输入，并暂存附件。"""
    ...
    if not trimmed and not is_skip and not saved_attachments:
        await message.answer(
            "前置条件可选：可继续输入条件（可同时发送附件），或发送“跳过”继续录入复现步骤：",
            reply_markup=_build_description_keyboard(),
        )
        return
...
async def on_task_defect_report_reproduction(message: Message, state: FSMContext) -> None:
    ...
    if not trimmed and not is_skip and not saved_attachments:
        ...
        await message.answer(
            "复现步骤可选：可继续输入步骤（可同时发送附件），或发送“跳过”继续录入现状：",
            reply_markup=_build_description_keyboard(),
        )
        return
    ...
    await state.set_state(TaskDefectReportStates.waiting_current_state)
    await message.answer(
        "请输入现状（可选），可直接发送图片/文件作为附件；若暂无可发送“跳过”继续（仅发送附件也会进入下一步）：",
        reply_markup=_build_description_keyboard(),
    )


@router.message(TaskDefectReportStates.waiting_current_state)
async def on_task_defect_report_current_state(message: Message, state: FSMContext) -> None:
    """报告缺陷：处理现状输入，并暂存附件。"""
    ...
    if not trimmed and not is_skip and not saved_attachments:
        await message.answer(
            "现状可选：可继续输入现状（可同时发送附件），或发送“跳过”继续录入预期效果：",
            reply_markup=_build_description_keyboard(),
        )
        return
    current_state, attachments_for_value = _build_step_content_with_attachments(
        message,
        raw_text=trimmed,
        saved_attachments=saved_attachments,
        field_label=DEFECT_CURRENT_STATE_LABEL,
    ...
    await state.set_state(TaskDefectReportStates.waiting_expected_effect)
    await message.answer(
        "请输入预期效果（可选），可直接发送图片/文件作为附件；若暂无可发送“跳过”继续（仅发送附件也会进入下一步）：",
        reply_markup=_build_description_keyboard(),
    )


@router.message(TaskDefectReportStates.waiting_expected_effect)
async def on_task_defect_report_expected_effect(message: Message, state: FSMContext) -> None:
    """报告缺陷：处理预期效果输入，并进入确认阶段。"""
    ...
    if not trimmed and not is_skip and not saved_attachments:
        await message.answer(
            "预期效果可选：可继续输入目标效果（可同时发送附件），或发送“跳过”进入确认创建：",
            reply_markup=_build_description_keyboard(),
        )
        return
    expected_effect, attachments_for_value = _build_step_content_with_attachments(
        message,
        raw_text=trimmed,
        saved_attachments=saved_attachments,
        field_label=DEFECT_EXPECTED_EFFECT_LABEL,
    )
    if attachments_for_value:
        pending = list(data.get("pending_attachments") or [])
        pending.extend(_serialize_saved_attachment(item) for item in attachments_for_value)
        await state.update_data(pending_attachments=pending)
    await state.update_data(expected_effect=expected_effect)
...
async def on_task_defect_report_confirm(message: Message, state: FSMContext) -> None:
    ...
    if extra_attachments or (extra_text and not is_cancel and not is_confirm):
        ...
        segment, attachments_for_value = _build_step_content_with_attachments(
            ...
            raw_text=segment_text,
            saved_attachments=extra_attachments,
            field_label=f"补充{DEFECT_EXPECTED_EFFECT_LABEL}",
        )
        pending = list(data.get("pending_attachments") or [])
        if attachments_for_value:
            pending.extend(_serialize_saved_attachment(item) for item in attachments_for_value)
        expected_effect = _append_step_content(expected_effect, segment)
        await state.update_data(pending_attachments=pending, expected_effect=expected_effect)
        data = await state.get_data()
        origin_task_id = data.get("origin_task_id")
        origin_task = await TASK_SERVICE.get_task(origin_task_id) if origin_task_id else None
        ...
        await message.answer(
            f"已记录补充的{DEFECT_EXPECTED_EFFECT_LABEL}/附件，请继续选择“确认创建”或“取消”。\n" + "\n".join(updated_lines),
            reply_markup=_build_confirm_keyboard(),
        )
        return
    ...
    current_state = (data.get("current_state") or "").strip()
    expected_effect = (data.get("expected_effect") or "").strip()
    description = _build_defect_description(
        precondition,
        reproduction,
    ...
    # 前置条件、复现步骤、现状与预期效果均可为空，仅校验关键上下文与标题。
