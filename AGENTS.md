...
| Telegram→tmux 投递确认约束       | 普通 Telegram 直聊启用 session JSONL 用户消息投递确认；tmux send-keys 成功但模型未确认消费时自动重试一次，仍未确认则明确提示失败。Codex `/goal` 是 TUI slash command，不复用普通用户消息确认，仍保留 worker marker 与禁用 latest fallback。                                                   | `bot.py`（锚点：`PROMPT_DELIVERY_CONFIRM_ENABLED`、`_session_contains_user_prompt_since`、`_confirm_or_retry_prompt_delivery`、`confirm_delivery=False`）；`tests/test_tmux_send_line.py`（锚点：`test_dispatch_prompt_retries_with_queue_when_user_prompt_not_confirmed`、`test_dispatch_prompt_reports_unconfirmed_after_retry`）；`tests/test_task_description.py`（锚点：`test_on_text_direct_prompt_enables_delivery_confirmation`、`test_goal_command_dispatches_objective_to_codex`、`test_goal_management_callbacks_dispatch_without_jsonl_delivery_confirmation`）                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Telegram→tmux 提示词规约前缀约束    | 普通非 slash 用户消息进入 tmux 前必须自动追加完整 `ENFORCED_AGENTS_NOTICE`，要求模型先读取全局/根目录/受影响子项目 AGENTS 规约、执行 vibe -> design -> develop 与 TDD 门禁，并以“以下是用户需求描述：”作为用户原文分隔；slash 命令与已内置规约的 Plan 实现提示不得重复追加。                                       | `bot.py`（锚点：`ENFORCED_AGENTS_NOTICE`、`_prepend_enforced_agents_notice`、`_handle_prompt_dispatch`）；`tests/test_agents_template_migration.py`（锚点：`test_enforced_notice_points_to_agents_md`、`test_enforced_notice_adds_user_requirement_header_before_prompt`）；`tests/test_task_description.py`（锚点：`test_prepend_enforced_agents_notice_cases`）；`tests/test_tmux_send_line.py`（锚点：`test_dispatch_prompt_retries_with_queue_when_user_prompt_not_confirmed`、`test_dispatch_prompt_reports_unconfirmed_after_retry`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
