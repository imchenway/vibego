...
| Worker 主菜单按钮收敛约束           | Worker 常驻底部键盘仅保留 `📋 任务列表` 与 `📟 命令管理`；`/session_live`、`/plan_mode`、`/goal` 通过 Telegram 命令菜单唤起会话实况、终端模式切换与 Codex goal 管理，主菜单渲染不再为了隐藏按钮探测 tmux。                                                                              | `bot.py`（锚点：`BOT_COMMANDS`、`_build_worker_main_keyboard`、`on_session_live_command`、`on_worker_plan_mode_command`、`on_goal_command`）；`tests/test_chat_menu_buttons.py`（锚点：`test_worker_keyboard_structure`、`test_worker_main_keyboard_does_not_probe_hidden_mode_buttons`、`test_worker_commands_menu_exposes_session_plan_and_goal_commands`、`test_worker_session_live_command_opens_session_list`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
