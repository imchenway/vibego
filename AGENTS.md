...
| Codex `/goal` 回传约束 | `/goal` 透传后必须保持 tmux 输入会话与 JSONL 回传会话同源；pointer 缺失时禁止扫描全局 latest rollout，避免串到 Codex App 会话。                                                                                  | `bot.py`（锚点：`allow_session_discovery_fallback=False`、`/goal 绑定 active thread`）；`tests/test_task_description.py`（锚点：`test_goal_dispatch_does_not_fallback_to_global_latest_session`） |
