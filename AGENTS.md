...
| Worker 主菜单按钮收敛约束           | Worker 常驻底部键盘仅保留两个高频入口：左侧 `📟 命令管理`，右侧 `🧭 PLAN MODE: ...`（Copilot 为 `🧭 MODE: ...`）；`📋 任务列表`、`/session_live`、`/goal` 通过 Telegram 命令菜单或原有文本命令唤起；右侧模式按钮会按当前终端状态刷新文案，`refresh_plan_mode_state=False` 时优先复用缓存。                                                                              | `bot.py`（锚点：`BOT_COMMANDS`、`_build_worker_main_keyboard`、`_resolve_worker_plan_mode_button_text`、`_resolve_worker_copilot_mode_button_text`、`on_worker_plan_mode_button`、`on_worker_plan_mode_command`）；`tests/test_chat_menu_buttons.py`（锚点：`test_worker_keyboard_structure`、`test_worker_keyboard_button_text`、`test_worker_main_keyboard_probes_visible_plan_mode_button`、`test_worker_main_keyboard_uses_cached_plan_mode_when_refresh_disabled`、`test_worker_main_keyboard_uses_explicit_plan_mode_state`、`test_worker_main_keyboard_uses_copilot_mode_button`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
