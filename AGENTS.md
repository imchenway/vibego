...
| 微信开发命令约束 | 通用命令提供 `wx-dev-preview` 生成二维码、`wx-auto-preview` 触发手机自动预览、`wx-dev-upload` 上传体验版；三者复用微信开发者工具 IDE HTTP 服务端口配置与小程序目录选择。 | `command_center/defaults.py`（锚点：`wx-dev-preview`、`wx-auto-preview`、`wx-dev-upload`）；`bot.py`（锚点：`WX_AUTO_PREVIEW_COMMAND_NAME`、`_is_wx_devtools_command`、`_build_wx_preview_prompt`）；`scripts/gen_preview.sh`（锚点：`WX_PREVIEW_ACTION=auto-preview`）；`README.md`（锚点：`微信开发命令端口配置`）；`tests/test_wx_preview_detection.py`（锚点：`test_wx_auto_preview_is_default_global_command`）；`tests/test_wx_preview_port_flow.py`（锚点：`test_gen_preview_auto_preview_mode_uses_auto_preview_without_qr`） |
