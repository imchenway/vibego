...
| Telegram PlanConfirm 三选项约束 | Codex 模型输出 `<proposed_plan>` 收口后，Telegram 必须展示与终端一致的三项：当前线程实现、清空上下文 fresh 线程实现、留在 Plan 模式；fresh 线程必须携带计划正文并禁用会话发现兜底。 | `bot.py`（锚点：`PLAN_CONFIRM_ACTION_FRESH`、`PlanConfirmSession.plan_text`、`_build_plan_confirm_keyboard`、`_build_plan_fresh_context_prompt`、`_restart_main_tmux_fresh_session`、`_restart_parallel_tmux_fresh_session`、`allow_session_discovery_fallback=False`）；`tests/test_plan_confirm_bridge.py`（锚点：`test_maybe_send_plan_confirm_prompt_uses_codex_three_option_keyboard`、`test_plan_confirm_fresh_context_restarts_main_tmux_and_dispatches_plan`、`test_parallel_plan_confirm_fresh_context_restarts_bound_parallel_tmux`） |
