...
| Telegram 长文本粘贴聚合约束         | 直接粘贴接近 Telegram 单条上限的长文本时，worker 会使用独立长窗口聚合慢到达分片；聚合后若超过 `TELEGRAM_MESSAGE_LIMIT`，再复用本地附件提示词发送到模型，避免多段文本分别进入 tmux。                                                                                                           | `bot.py`（锚点：`TEXT_PASTE_LONG_CHUNK_AGGREGATION_DELAY`、`PendingTextPasteState.long_chunk_seen`、`_text_paste_finalize_delay_seconds`、`_maybe_enqueue_text_paste_message`、`_handle_prompt_dispatch`、`_persist_text_paste_as_attachment`）；`tests/test_task_description.py`（锚点：`test_text_paste_near_limit_chunks_wait_longer_than_prefix_window`、`test_text_paste_aggregation_injects_single_combined_message`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
