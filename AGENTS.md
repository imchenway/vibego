...
- 任何一次开始动手修改前，必须得到用户的明确许可允许进入下一阶段，直到 develop 阶段完成后自动回退到 vibe 阶段，禁止连续处在开发阶段中。
...
- 必须在收到用户明确的可以开始开发设计指令后，才可以进入 design 阶段。
- 最后切换阶段前自行整理出本次阶段的结论 ，防止在后续的任务执行中遗漏，必要时更新文档，写入当前目录的 docs
...
- 必须在收到用户明确的可以开始修改文件的指令后，才可以进入 develop 阶段。
- 最后切换阶段前自行整理出本次阶段的结论 ，防止在后续的任务执行中遗漏，必要时更新文档，写入当前目录的 docs 文件夹中。
...
- 最后切换阶段前自行整理出本次阶段的交付物及后续待用户执行的操作内容 ，防止在后续的任务执行中遗漏，必要时更新文档，写入当前目录的 docs 文件夹中。
