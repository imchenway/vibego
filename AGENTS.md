...
| Codex `/goal` 回传约束 | `/goal` 透传后必须保持 tmux 输入会话与 JSONL 回传会话同源；pointer 缺失或 pointer 指向缺少 worker marker 的 Codex 会话时必须 fail-closed，避免串到 Codex App 会话。                                                        | `bot.py`（锚点：`allow_session_discovery_fallback=False`、`_read_required_session_marker`）；`scripts/session_binder.py`（锚点：`--required-marker`）；`scripts/start_tmux_codex.sh`（锚点：`prepare_codex_model_instructions_file`）；`tests/test_task_description.py`（锚点：`test_goal_dispatch_rejects_pointer_without_worker_marker`）；`tests/test_session_binder_codex.py` |
