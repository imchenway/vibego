...
- 在保证不改变和遗漏对终端的回复的前提下，为了尽可能的保留长期记忆，切换阶段时都需要旁路更新文档，同时写入当前目录的 docs
  文件夹中，文件名格式必须是：任务编号_任务描述.md（用户提示词不存在任务编号时，使用 TASK_YYYYMMDD_XXX 格式自动编号）
  ，如有当前任务编码已经有文档，优先写入当前文档。
- develop 阶段完成后自动回退到 vibe 阶段，禁止连续处在 develop 阶段中。

## vibe 阶段（需求调研/问题分析） - 严禁修改代码｜允许访问网络｜自定义扫描范围
...
## design 阶段（开发设计 + 用例设计） - 严禁修改代码｜允许访问网络｜自定义扫描范围
...
## develop 阶段（需求开发/问题修复） - 可以修改/删除文件｜允许访问网络｜自定义扫描范围
...
- 冒烟：对健康检查与关键 API 做 200/超时/鉴权三类断言，生成 JaCoCo 并按行/分支阈值硬闸。
