...
| Telegram→tmux 投递确认约束 | 普通 Telegram 直聊与 Codex `/goal` 入口启用 session JSONL 投递确认；tmux send-keys 成功但模型未确认消费时自动重试一次，仍未确认则明确提示失败。 | `bot.py`（锚点：`PROMPT_DELIVERY_CONFIRM_ENABLED`、`_session_contains_user_prompt_since`、`_confirm_or_retry_prompt_delivery`、`confirm_delivery=True`）；`tests/test_tmux_send_line.py`（锚点：`test_dispatch_prompt_retries_with_queue_when_user_prompt_not_confirmed`、`test_dispatch_prompt_reports_unconfirmed_after_retry`）；`tests/test_task_description.py`（锚点：`test_on_text_direct_prompt_enables_delivery_confirmation`、`test_goal_command_dispatches_objective_to_codex`） |
