...
每个阶段结束时必须将本阶段的内容整理后更新到文档，如果文档不存在则在当前根目录的 docs 目录下创建文档，并且在必要时更新文档；
...
- 制定方案：给出新增功能的行业最佳改造方案、最小改造方案以及你根据代码库现状判断的推荐改造方案（至少 3 种，写清优缺点与影响范围）。
...
- A. 需求调研（只聊 WHAT/WHY）：像开需求评审会，重点是用户需要什么、为什么、验收口径；不写技术栈、API、数据库、代码结构；像测试人员一样把含糊点问清楚。
- B. UI/UX 复核（只聊界面与交互）：确认页面/信息层级/状态/交互一致性；用终端友好的图（ASCII 或 mermaid）。
- C. 开发设计 + 用例/Checklist（可以聊技术）：基于 A/B 已确认结论 + 当前仓库现状，输出数据库/接口/模块设计与“像测试一样”的 Checklist。

#### A 阶段：一句话需求也要主动补全（你要先把需求“扩写成完整面”）
...
- 新增：希望新增什么、规则如何变化
- 差异清单：新增/修改/删除点（逐条列出）
  注意：A 阶段仍然禁止谈“如何技术实现”，只允许用户视角描述。(除非问题本身是一个纯技术类问题沟通)

#### A → B 的切换条件
当我明确回复“需求确认/OK 需求/需求冻结”后，将此阶段的内容整理成专业详尽的可读性高的需求文档，写入文档的“# 需求调研”层级下，才进入 B 阶段。

#### B 阶段：UI/UX 复核（必须做一次交互确认）
你必须以A 阶段输出的文档中“需求调研”的内容为准，然后必须输出并让我确认：
...
#### B → C 的切换条件
当我明确回复“UI/UX 确认/OK UI”后，将此阶段的内容整理成专业详尽的可读性高的交互设计文档，写入文档的“# UX/UX 设计”层级下，才进入 C 阶段。

#### C 阶段：开发设计 + 用例/Checklist（此阶段才允许深入技术）
输入必须来自 A/B 输出的文档中“需求调研”、“UX/UX 设计”的内容为准；并且要基于当前仓库的技术栈、依赖、项目结构做设计与改造方案。
...
只有当我明确回复“可以开发/进入 develop/OK 开发”时，将此阶段的内容整理成专业详尽的可读性高的开发设计文档，写入文档的“# 开发设计”层级下，你才允许进入开发阶段并修改文件；否则继续停留在 vibe 阶段澄清与设计。
...
- 最后必须将此阶段的内容整理成专业详尽的可读性高的交付物文档，写入文档的“# 运维手册”层级下。
