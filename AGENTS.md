...
- 所有结论、问题与设计必须全部通过交互得到明确答复后，才可以申请进入 design 阶段，必须在收到用户明确的可以开始开发设计指令后，才可以进入
  design 阶段。
- 为防止在后续的任务执行中遗漏，完成该阶段时需更新文档，整理出本次阶段的结论，写入当前目录的 docs
...
- 所有结论、问题与设计必须全部通过交互得到明确答复后，才可以申请进入 develop 阶段，必须在收到用户明确的可以开始修改文件的指令后，才可以进入
  develop 阶段。。
- 为防止在后续的任务执行中遗漏，完成该阶段时需更新文档，整理出本次阶段的结论 ，写入当前目录的 docs
...
- 为防止在后续的任务执行中遗漏，完成该阶段时需更新文档，整理出本次阶段的交付物及后续待用户执行的操作内容，写入当前目录的
