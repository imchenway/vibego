...
- 普通为什么/怎么做追问默认简洁文本回答；普通概念问答、安装升级说明、轻量决策和非视觉化追问默认使用简洁文本。
...
- 用户明确要求 HTML/图形化，或确实需要用图表达复杂关系时，才生成或更新项目内单文件 HTML；普通概念问答、安装升级说明、轻量决策和非视觉化追问默认使用简洁文本。
...
- 明确要求画图、图形化、HTML 图，或需要把复杂技术/业务逻辑、关系结构或状态流转可视化时，优先触发 vibe-diagram 生成单文件 HTML；核心理念是一图胜千言；AGENTS 只判断何时触发，具体制图规则以 vibe-diagram 为准。
- 普通概念问答、安装升级说明、轻量决策和非视觉化追问默认使用简洁文本；不要为了回答每个原因解释、步骤补充或追问而强制生成 HTML。
